...
| V16              | **Déséquilibre & seuils** : courbe précision-rappel, seuil de décision ajustable avec matrice de coûts, pondération de classes, courbe de calibration                                                                                                                                                                                                                                | Les vrais jeux sont déséquilibrés ; l'accuracy y ment                                                    |
| V17              | **Data Studio 3 : jointures & anomalies** : jointure de deux fichiers sur clé commune (taux de correspondance, orphelines), détection d'anomalies multivariée (isolation forest maison seedée) — intégrées à la recette rejouable                                                                                                                                                    | La préparation réelle commence par croiser deux fichiers ; l'anomalie multivariée voit ce que Tukey rate |
| V18              | **Analyse par segments** : métriques du modèle par sous-groupe avec écarts mis en évidence, intégrée au rapport et aux artefacts du run                                                                                                                                                                                                                                              | « Où mon modèle échoue-t-il ? » — porte d'entrée honnête vers l'équité                                   |
