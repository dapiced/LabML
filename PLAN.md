...
| V12              | **Chat génératif consenti** : Cloudflare Pages Function (même repo) en proxy vers l'API Anthropic — la clé vit en secret Cloudflare, jamais dans le navigateur ; le LLM traduit la question en intention exécutée **localement** par le moteur V6 (seuls la question et le schéma des colonnes partent, jamais les données) ; écran de consentement explicite ; dégradation propre si le secret n'est pas configuré | Exige UNE action du propriétaire : ajouter la clé en secret — décision produit à valider                   |
| V13              | **Runs complets** : tuning, Shapley, exploration et prévision intégrés à l'historique IndexedDB, au rapport HTML et aux liens de partage                                                                                                                                                                                                                                                                            | Les artefacts V5–V8 ne survivent pas encore au run                                                         |
| V14              | **Prérendu généralisé** : coquilles statiques pour toutes les routes (approche V9 étendue), viser /ml ≥ 0,90 sans SSR complet                                                                                                                                                                                                                                                                                       | Le dernier écart Lighthouse                                                                                |
