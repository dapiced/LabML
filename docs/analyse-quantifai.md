...
**Enseignement clé :** le "ML Lab" gratuit est une _section de la landing_, pas une application
...
3. **Entraînement** — bouton unique "Train Models" ; étapes affichées pendant le run :
   _Detecting the task type → Holding back a test split → Training a set of models → Ranking
   against a baseline_ ; badge "running locally, no requests". Après le run, le bouton devient
   "Train again ↻".
...
- **Body** : Inter. **Mono** : JetBrains Mono — utilisée massivement comme _voix technique_ :
...
- Preuve privacy théâtralisée : faux panneau devtools "Network — 0 requests during training"
  (`quantifai-03`). L'argument phare : _"That is the proof, not a promise."_
...
sombre. LabML garde _les patterns_ qui fonctionnent (eyebrows mono, cartes douces, baseline
